...
object Logger {
    ...
    private fun getDefaultLogPath(): String {
        return if (::appContext.isInitialized) {
            ...
            // 备用方案，但这种情况不应该发生
            "/storage/emulated/0/Android/data/com.sakura.quantumplanet/logs"
        }
    }
    ...
    fun setLogEnabled(enabled: Boolean) {
        try {
            ...
            mmkv.encode(KEY_LOG_ENABLED, enabled)
            
            if (enabled) {
                internalLog(INFO, "Logger", "日志功能已开启", false)
            } else {
                internalLog(INFO, "Logger", "日志功能已关闭", false)
            }
        } catch (e: Exception) {
            Log.e("Logger", "设置日志开关失败: ${e.message}")
        }
    }
    ...
    fun isLogEnabled(): Boolean {
        return try {
            val mmkv = MMKV.defaultMMKV()
            mmkv.decodeBool(KEY_LOG_ENABLED, DEFAULT_LOG_ENABLED)
        } catch (e: Exception) {
            Log.e("Logger", "获取日志开关状态失败: ${e.message}")
            DEFAULT_LOG_ENABLED
        }
    }
